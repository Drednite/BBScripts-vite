...
export async function main(ns: NS) {
  ...
  var target = ns.getServer(ns.args[0].toString());
  ...
  var serverMaxMoney = target.moneyMax!;
  var serverMaxRAM;
  ...
  if(serverToHackFrom.hostname == "home"){
    serverMaxRAM -= Math.min(HOMERAMRES, serverMaxRAM*.1);
  }
  ...
  while (timout < 5) {
      ...
      if (3 < (possibleThreads = Math.floor((serverMaxRAM - serverToHackFrom.ramUsed) / growScriptRAM))) {
          ...
          currentServerMoney = target.moneyAvailable!;
          serverMaxMoney = target.moneyMax!;
          moneyThresh = serverMaxMoney * 0.9;
          ns.printf("Money: $%s / $%s\n", ns.formatNumber(currentServerMoney), ns.formatNumber(serverMaxMoney));
          currentServerSecurity = target.hackDifficulty!;
          ...
          try {
              ...
              if (currentServerSecurity > securityThresh) {
                  maxGrowThreads = Math.ceil(ns.growthAnalyze(target.hostname, Math.max(moneyThresh / currentServerMoney, 1.01), serverToHackFrom.cpuCores));
                  useThreadsGrow = Math.min(Math.ceil(possibleThreads * .2), maxGrowThreads);
                  useThreadsWeaken1 = possibleThreads - useThreadsGrow;
                  ns.exec(growScript, serverToHackFrom.hostname, {threads: useThreadsGrow, temporary: true}, target.hostname, 0);
                  ns.exec(weakenScript, serverToHackFrom.hostname, {threads: useThreadsWeaken1, temporary: true}, target.hostname, 0);
                  timout = 0;
                  ...
                  if (currentServerMoney > 0) {
                      ...
                      maxGrowThreads = possibleThreads;
                  ...
                  while (maxHackFactor < 0.999 &&
                      Math.floor((possibleThreads - (useThreadsHack = Math.floor(ns.hackAnalyzeThreads(target.hostname, currentServerMoney * maxHackFactor))) - Math.ceil(useThreadsHack / 25)) * growWeakenRatio)
                          > Math.ceil(ns.growthAnalyze(target.hostname, serverMaxMoney / (serverMaxMoney * (1 - maxHackFactor)), serverToHackFrom.cpuCores))) {
                  ...
                  useThreadsHack = Math.max(Math.floor(ns.hackAnalyzeThreads(target.hostname, currentServerMoney * maxHackFactor)), 1); // Forgot this in the first version.
                  ...
                  for (i = 0; i < batches; i++) {
                      ns.exec(weakenScript, serverToHackFrom.hostname, {threads: useThreadsWeaken1, temporary: true}, target.hostname, 0, 0 + 2 * i);
                      sleepTime = 2 * sleepDelay;
                      ns.exec(weakenScript, serverToHackFrom.hostname, {threads: useThreadsWeaken2, temporary: true}, target.hostname, sleepTime, 1 + 2 * i); // Second weaken script runs after the first
                      sleepTime = sleepTimeWeaken - sleepTimeGrow + sleepDelay;
                      ns.exec(growScript, serverToHackFrom.hostname, {threads: useThreadsGrow, temporary: true}, target.hostname, sleepTime, i); // Grow script ends before second weaken script
                      sleepTime = sleepTimeWeaken - sleepTimeHack - sleepDelay;
                      ns.exec(hackScript, serverToHackFrom.hostname, {threads: useThreadsHack, temporary: true}, target.hostname, sleepTime, i); // Hack script ends before first weaken script
