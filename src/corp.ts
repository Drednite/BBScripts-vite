  ...
  while (true) {
    ...
    switch (state) {
      ...
      case 13:  //  purchase more levels of smart storage and smart factories
        if (purchaseUpgrade(upgradeNames.SmartStorage, false, 10) && purchaseUpgrade(upgradeNames.SmartFactories, false, 10)) state++;
        break;
      ...
      */
      ...
      case 21:  //  purchase exporting
        if (!corp.hasUnlock(unlockNames.Export)) {
          ...
          try{corp.purchaseUnlock(unlockNames.Export);}
          catch{break}
        }
  ...
  */
  function expandOffice(div = "", size = 0, justS12 = false) {
    ...
    for (let city of Object.values(cityNames)) {
      ...
      let needed = size - corp.getOffice(div, city).size;
      if (needed > 0) {
      ...
      if (corp.getOffice(div, city).size >= size) { finished++; }
    }
   ...
   */
  function assignJobs(div = "", operations = 0, engineers = 0, business = 0, managers = 0, researchers = 0, justS12 = false) {
  ...
  */
  function maintainProducts() {
    ...
    for (let div of corp.getCorporation().divisions) {
      ...
      for (let n of currentProducts) {
        ...
        let product = corp.getProduct(div, cityNames.Sector12, n);
  ...
  function maintainUpgrades() {
    for (let upgrade of Object.values(upgradeNames)) {
      ...
      } else if ((corp.getCorporation().funds * 0.10) > corp.getUpgradeLevelCost(upgrade)) {
