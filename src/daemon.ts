...
export async function attack(ns: NS, target: string, next: string): Promise<true> {
  ...
  const delayTime = Math.min(ns.getWeakenTime(target) / powerlessServers.length, ns.getWeakenTime(target) - ns.getGrowTime(target)) + 200;
  const tasks = [];
  if ((nextLevel < ns.getHackingLevel()) && ns.getServerMoneyAvailable(target) > .9 * maxMoney) {
    return ns.sleep(60);
  }
  ...
  if (next === "END")
  return ns.sleep(60);
  while ((nextLevel > ns.getHackingLevel()) || ns.getServerMoneyAvailable(target) < .9 * maxMoney) {
    await ns.sleep(1000);
  }
  tasks.forEach((task) => {
    ns.kill(task);
  });
  while (ns.scriptRunning("weaken.js", "home") || ns.scriptRunning("grow.js", "home") || ns.scriptRunning("hack.js", "home")) {
    await ns.sleep(1000);
  }
