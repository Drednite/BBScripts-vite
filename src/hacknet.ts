...
export async function main(ns: NS) {
  ...
  let delayTime = 1000;
  let maxNodes = Math.min((typeof flags.maxNodes == "number") ? flags.maxNodes : Number.MAX_SAFE_INTEGER, ns.hacknet.maxNumNodes());
  ...
  while (Date.now() - ns.getResetInfo().lastAugReset < 1.8e6) {
    ...
    if (ownedNodes > maxNodes - 1) {
      ...
      let lastNode = ns.hacknet.getNodeStats(maxNodes - 1);
    ...
    for (let i = 0; i < ownedNodes; i++) {
      ...
      nodeList += "|n:" + ns.sprintf("%2d", i) + "|LV:" + ns.sprintf("%3d", node.level) + "|RAM:" + ns.sprintf("%5s", ns.formatRam(node.ram, 0)) + "|CR:" + ns.sprintf("%3d", node.cores) + "|\n";
      let upgrades = [
...
*/
async function waitForMoney(ns: NS, targetMoney: number, delayTime: number, thresholdMultiplier: number, nodeList: string) {
  while (ns.getServerMoneyAvailable("home") < targetMoney * thresholdMultiplier) { // (ns.getMoneySources().sinceInstall.hacknet + ns.getMoneySources().sinceInstall.hacknet_expenses)  /
    ns.clearLog();
    const moneySources = ns.getMoneySources().sinceInstall
    ns.printf(nodeList + 
    "|Income: %20s|\n|Expenses: %18s|\n|Profit: %20s|\n|Target: %20s|", ns.formatNumber(moneySources.hacknet + moneySources.codingcontract), ns.formatNumber(moneySources.hacknet_expenses), ns.formatNumber(moneySources.hacknet + moneySources.codingcontract + moneySources.hacknet_expenses), ns.formatNumber(targetMoney * thresholdMultiplier));
    ...
    if(!constantCont && (ns.hacknet.hashCapacity() * 250000) > targetMoney * thresholdMultiplier - ns.getServerMoneyAvailable("home")){
...
export function levelUpgradeProfit(currentLevel: number, usedRam: number, currentRam: number, currentLevelCore: number) {
  return (calculateHashGainRate(currentLevel+1, usedRam, currentRam, currentLevelCore) - calculateHashGainRate(currentLevel, 0, currentRam, currentLevelCore)) * 250000;
}
export function ramUpgradeProfit(currentLevel: number, usedRam: number, currentRam: number, currentLevelCore: number) {
  return (calculateHashGainRate(currentLevel, usedRam, currentRam*2, currentLevelCore) - calculateHashGainRate(currentLevel, 0, currentRam, currentLevelCore)) * 250000;
}
export function coreUpgradeProfit(currentLevel: number, usedRam: number, currentRam: number, currentLevelCore: number) {
  return (calculateHashGainRate(currentLevel, usedRam, currentRam, currentLevelCore+1) - calculateHashGainRate(currentLevel, 0, currentRam, currentLevelCore)) * 250000;
}

export function calculateHashGainRate(level: number, ramUsed: number = 0, maxRam: number, cores: number): number {
  const baseGain = .001 * level;
