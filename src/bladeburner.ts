...
export async function main(ns: NS): Promise<void> {
  ...
  const cities = [CityName.Aevum, CityName.Chongqing, CityName.Ishima, CityName.NewTokyo, CityName.Sector12, CityName.Volhaven];
  
  if(!sin.getOwnedAugmentations().includes("The Blade's Simulacrum")){
    sin.stopAction();
  }
  ...
  */
  function bestAnalysis(): [string, string]{
    ...
    if(current*2 < max){  // are you just tired?
      return ["general", "Training"];
    }
    if(bb.getActionEstimatedSuccessChance("operation", "Undercover Operation")[0] > .99 
    && bb.getActionCountRemaining("operation", "Undercover Operation") > 0){
      return ["operation", "Undercover Operation"]
    }
    else if(bb.getActionEstimatedSuccessChance("operation", "Investigation")[0] > .99 
    && bb.getActionCountRemaining("operation", "Investigation") > 0){
      return ["operation", "Investigation"]
    }
    else if(bb.getActionEstimatedSuccessChance("contract", "Tracking")[0] > .99 
  ...
  */
  function diplomacy(): [string, string]{
    const city = bb.getCity();
    if(bb.getActionEstimatedSuccessChance("operation", "Assassination")[0] > .99 && bb.getActionCountRemaining("operation", "Assassination") > 0){
      return ["operation", "Assassination"];
    }
    else if(bb.getActionCountRemaining("operation", "Stealth Retirement Operation") > 0 && bb.getCityEstimatedPopulation(city) > popThreshold && bb.getActionEstimatedSuccessChance("operation", "Stealth Retirement Operation")[0] > .99){
  ...
  */
  function upgrades(){
    ...
    while(cont){
      ...
      else if(bb.getSkillLevel("Overclock") < 90 && bb.getSkillUpgradeCost("Overclock") < points){  // if struggling with stamina,
        points -= bb.getSkillUpgradeCost("Overclock");                                              // don't speed up rate of consumption
        cont = bb.upgradeSkill("Overclock");
      }
      
      if(bb.getSkillLevel("Blade's Intuition")*4 < bb.getSkillLevel("Digital Observer")*3 && bb.getSkillUpgradeCost("Blade's Intuition") < points){
      ...
      else if(bb.getSkillLevel("Short-Circuit") >= bb.getSkillLevel("Cloak") && bb.getSkillUpgradeCost("Cloak") < points){
