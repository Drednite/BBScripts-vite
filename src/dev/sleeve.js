...
const argsSchema = [
  ...
  ['buy-cooldown', 600 * 1000], // Must wait this may milliseconds before buying more augs for a sleeve
...
async function manageSleeveAugs(ns, i, budget) {
  ...
  const purchaseUpdate =
    `sleeve ${i} can afford ${batchCount.toFixed(0).padStart(2)}/${availableAugs[i].length
      .toFixed(0)
      .padEnd(2)} remaining augs ` +
  ...
  if (
    ...
    let strAction = `Purchase ${batchCount}/${
      availableAugs[i].length
    } augmentations for sleeve ${i} at total cost of ${formatMoney(batchCost)}`;
...
async function getAllSleeves(ns, numSleeves) {
  return await getNsDataThroughFile(ns, `ns.args.map(i => ns.sleeve.getSleeve(i))`, `/Temp/sleeve-getSleeve-all.txt`, [
...
async function mainLoop(ns) {
  ...
  if (canTrain && task.some((t) => t?.startsWith('train')) && !options['disable-spending-hashes-for-gym-upgrades'])
    if (
      await getNsDataThroughFile(ns, 'ns.hacknet.spendHashes("Improve Gym Training")', '/Temp/spend-hashes-on-gym.txt')
    )
  ...
  if (sleeve.shock > 0 && options['shock-recovery'] > 0) {
    ...
    if (shockChance[i] < options['shock-recovery'])
      return shockRecoveryTask(
        sleeve,
        i,
        `there is a ${(options['shock-recovery'] * 100).toFixed(
          1,
        )}% chance (--shock-recovery) of picking this task every minute until fully recovered.`,
      );
  }
  ...
  return [
    ...
    /*   */ `committing ${crime} with chance ${(successChance * 100).toFixed(2)}% because ${reason}` +
      /*   */ (options.crime || crime == 'homicide'
        ...
        : /*   */ ` (Note: Homicide chance would be ${(
            (await calculateCrimeChance(ns, sleeve, 'homicide')) * 100
          ).toFixed(2)}%)`),
  ];
}
