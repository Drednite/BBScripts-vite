    ...
    if (TIME_TRACKING && performance.now() - started > BENCH_TIME) {
      ...
      ns.tprint(
        "FAIL: It's been an hour and the current balance is $" +
          ns.formatNumber(balance) +
          ' initial funds were $' +
          ns.formatNumber(initialFunds) +
          ' profit: ' +
          (1 - (balance / initialFunds) * 100).toFixed(2) +
          '%',
      );
...
function BuyStonks(ns, log) {
  ...
  for (const stonk of stonks) {
    ...
    if (!g_tixMode && stonk.snapshots.length < LOG_SIZE) continue;
...
function ReportCurrentSnapshot(ns, stonks) {
  ...
  for (const stonk of stonks) {
    ...
    line.push({ color: 'white', text: ns.formatNumber(stonk.nbShares + stonk.nbShorts).padStart(9) });
    line.push({ color: 'white', text: ns.formatNumber(stonk.GetPricePaid()).padStart(9) });
  ...
  if (!g_tixMode && stonks.length > 0 && snaps < LOG_SIZE)
    ns.print(
      'WARN: Script running in pre-4S data mode (we need ' +
        LOG_SIZE +
        ' prices in the log before doing any trading): ' +
        snaps +
        '/' +
        LOG_SIZE,
    );
...
function UpdateHud(ns, totalWorth) {
  ...
  try {
    ...
    values.push(ns.formatNumber(totalWorth));
