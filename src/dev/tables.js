...
export async function main(ns) {
  ...
  data.push([
    ...
    {
      style: { color: 'white', textDecorationLine: 'line-through', textDecorationStyle: 'solid' },
...
function PrintBorder(ns, columns, style, printfunc = ns.print) {
  ...
  for (let c = 0; c < columns.length; c++) {
    ...
    if (c == columns.length - 1) printStack.push('white', style[CLOSER]);
    else printStack.push('white', style[SEPARATOR]);
  }
...
function PrintLine(ns, columns, data, style, printfunc = ns.print, highlight) {
  ...
  for (let c = 0; c < columns.length; c++) {
    if (data[c].style != undefined) printStack.push({ style: data[c].style }, data[c].text.padEnd(columns[c].width));
    else if (data[c].color != undefined)
      printStack.push(CreateStyle(data[c].color, highlight), data[c].text.padEnd(columns[c].width));
    else printStack.push(CreateStyle('white', highlight), data[c].toString().padEnd(columns[c].width));
    printStack.push('white', style[0][BAR]);
  }
  ...
  for (let i = 0; i < formattedData.length; i++) {
    ...
    for (var j = 0; j < col.length; j++) {
      ...
      if (typeof formattedData[i][col[j]] == 'number') {
        ...
        if (number > 0) tabCell.innerHTML = addHtmlWithColor('a', fmt.format(number), 'green');
        else if (number == 0) tabCell.innerHTML = addHtmlWithColor('a', fmt.format(number), 'white');
        else if (number < 0) tabCell.innerHTML = addHtmlWithColor('a', fmt.format(number), 'red');
      } else {
