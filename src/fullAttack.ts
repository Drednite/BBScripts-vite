...
export async function main(ns: NS) {
  ...
  if (flags.help) {
    const message =
      'INFO: starts masterHacks to utilize the RAM of all empty servers to attack one target\n' +
      'run ' +
      ns.getScriptName() +
      ' {--target server} {--hacker script} {--home} {--help} \n' +
  ...
  let serverList = await getAllServers(ns);
  const hacker = flags.hacker.toString();
  const hackerRam = ns.getScriptRam(hacker, 'home');
  let target = flags.target.toString();
  ...
  serverList = serverList.filter((server) => {
    return ns.hasRootAccess(server) && server != 'home' && ns.getServerMaxRam(server) >= hackerRam;
  });
  serverList.sort((a, b) => {
