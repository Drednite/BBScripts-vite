...
export async function main(ns: NS) {
  ...
  const emptyServers = [], powerlessServers = [];
  let keys = 0;
  ...
  serverList.sort((a, b) =>
  ns.getServerRequiredHackingLevel(a) - ns.getServerRequiredHackingLevel(b)
  );
  serverList.sort((a, b) =>
  ns.getServerNumPortsRequired(a) - ns.getServerNumPortsRequired(b)
  );
    ...
    if (ns.getServerMaxRam(node) > 4 && node != "home") {
      if (ns.getServerMaxMoney(node) > 0) {
        if (ns.getHackingLevel() < level * 2) {
          ...
          while (!ns.exec(hacker, "home", 1, node))
          await ns.sleep(waitTime);
          ns.scp(mark, node);
        }
        catch {
      ...
      if (ns.ps("home").some(isHosting, node) && ns.getHackingLevel() > level * 2)
      powerlessServers.push(node);
    }
  }
  if(flags.o){
  ...
  while (emptyServers.length > 0 && powerlessServers.length > 0) {
    ...
    if (target && host) {
      while (!ns.exec(hacker, "home", 1, target, host))
      await ns.sleep(waitTime);
      ns.scp(mark, target);
    }
  }
